...
EigenLayer middleware state bridge to L2 using ZK proofs. Implements a "mimic" contract pattern that replicates EigenLayer's registry coordinator functionality on L2 by verifying state proofs from L1.
...
|-------|----------|---------|
| L1 (Ethereum/Sepolia) | `MiddlewareShim` | Snapshots EigenLayer operator state |
| L2 | `RegistryCoordinatorMimic` | Verifies and stores state using SP1 Helios proofs |

### Key Contracts

**MiddlewareShim** (`contracts/src/MiddlewareShim.sol`)
...
**RegistryCoordinatorMimic** (`contracts/src/RegistryCoordinatorMimic.sol`)
...
- Uses direct assembly to set storage array lengths for gas efficiency
...
| `optimism` | RLP encoding and Merkle trie verification |
| `forge-std` | Testing framework |

**Import Remappings** (from `contracts/foundry.toml`):
```
...
2. Set required variables in `contracts/.env`:
   - `PRIVATE_KEY`: Deployer private key
   - `RPC_URL`: RPC endpoint for deployment
   - `ETHERSCAN_API_KEY`: For contract verification
   - `REGISTRY_COORDINATOR`: EigenLayer registry coordinator address
...
> **Note**: `DeployEnvironment.s.sol` has a compilation error referencing undefined constants. This doesn't affect core contracts.
...
Docker-based end-to-end testing infrastructure that simulates a full L1/L2 bridge deployment.
...
All scripts source `e2e/docker/scripts/config.sh` which:
...
See `e2e/envs/bls-testnet.example.env`. Key variables:
...
- **Build error**: `DeployEnvironment.s.sol` references undefined constants (doesn't affect core contracts)
- **Library linking**: `QuorumBitmapHistoryLib` is an external library requiring separate deployment; multi-chain scripts don't support library linking
- **Test listing**: Forge test listing may fail due to compilation errors but core contracts still work
- **Single quorum**: Most logic hardcoded for quorum 0 (`hex"00"`); multi-quorum support is limited

## Architecture Notes

| Aspect | Current State | Notes |
|--------|---------------|-------|
| Quorum handling | Hardcoded for quorum 0 | Multi-quorum is TODO |
| State updates | Full replacement | Incremental updates are TODO |
| Staleness | No enforcement | Older valid proofs are accepted |
| Gas limits | Potential issues | `getMiddlewareData()` may exceed block gas limit with many operators |
